...
@section('content')
    ...
    @php
        ...
        foreach($userGroups  as $key => $userGroup) {
            $status = $statusChange->returnStatusBullets("user_groups", $userGroup->status, $userGroup->id);
            $data[$key] = [
                $userGroup->id,
                $userGroup->name,
                $status,
                '<nobr><a href="'. route('admin.user-groups.edit', $userGroup->id).'" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar">
                ...
                <a href="'.route('admin.user-groups.show', $userGroup->id) .'" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Detalhes">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary card-outline">
                <div class="card-header"> <a href="{{ route('admin.user-groups.create') }}" class="btn btn-sm btn-primary">
                    <i class="fa fa-lg fa-fw fa-file"></i> Adicionar
                </a></div>
                <div class="card-body">
                    <x-adminlte-datatable id="table-user-group" :heads="$heads" head-theme="light" hoverable bordered with-buttons>
