...
<x-adminlte-input name="cep" label="CEP:*" value="{{ $address ? $address->cep : '' }}" class="cep-mask" placeholder="00000-000" enable-old-support>
    <x-slot name="bottomSlot">
        <a href="https://buscacepinter.correios.com.br/app/endereco/index.php" target="_blank">Não sei o CEP</a>
    </x-slot>
</x-adminlte-input>

<x-adminlte-input name="address" label="Endereço:*" value="{{ $address ? $address->address : '' }}" enable-old-support/>

<x-adminlte-input name="number" label="Número:*" value="{{ $address ? $address->number : '' }}" enable-old-support/>

<x-adminlte-input name="complement" label="Complement:" value="{{ $address ? $address->complement : '' }}" enable-old-support/>

<x-adminlte-input name="neighborhood" label="Bairro:" value="{{ $address ? $address->neighborhood : '' }}" enable-old-support/>

<x-adminlte-select name="country_id" label="País:*" enable-old-support>
...
<x-adminlte-select name="city_id" label="Cidade:" enable-old-support>
    @foreach($cities as $city)
        <option value="{{ $city->id }}" {{ isset($address->city_id) && $city->id == $address->city_id ? "selected" : "" }}>{{ $city->name }}</option>
    @endforeach
</x-adminlte-select>

<x-adminlte-input name="phone" label="Telefone:" value="{{ $address ? $address->phone : '' }}" class="phone-mask" placeholder="(00) 00000-0000" enable-old-support/>

<a href="{{ route('admin.addresses.index', $customer->id) }}" class="btn btn-warning"><i class="fa fa-times"></i> Cancelar</a>
