    ...
    <div class="col-md-12">
        ...
        <x-adminlte-select name="parent_id" label="Pai:">
            @foreach ($modules as $mod)
                <option value="{{ $mod->id }}"
                    {{ isset($module->parent_id) && $mod->id == $module->parent_id ? 'selected' : '' }}>
                    {{ $mod->name }}</option>
            @endforeach
        </x-adminlte-select>

        <x-adminlte-input name="name" label="Nome:*" value="{{ $module->name ?? '' }}" enable-old-support />

        <x-adminlte-input name="icon" label="Ícone:" value="{{ $module->icon ?? '' }}" enable-old-support />

        <x-adminlte-input name="slug" label="Apelido:" value="{{ $module->slug ?? '' }}" enable-old-support />

        <x-adminlte-input type="number" name="order" label="Ordem:*" value="{{ $module->order ?? '' }}"
            enable-old-support />
