    ...
    <div class="row">
        ...
        <div class="col-md-4">
            ...
            <div class="row">
                <div class="col-md-12">
                    <form method="POST"
                        action="{{ route('admin.customers.orders.update', [$order->contact_id, $order->id]) }}">
                        @method('PUT')
                        @csrf
                        <input type="hidden" name="type" value="order">
                        <x-adminlte-input name="tracking_code" label="Código Rastreio:"
                            value="{{ $order->tracking_code ?? '' }}" enable-old-support />
...
<div class="row">
    <div class="col-md-12">
        <form method="POST" action="{{ route('admin.customers.orders.update', [$order->contact_id, $order->id]) }}">
            @method('PUT')
            @csrf
            ...
            <x-adminlte-select name="order_status_id" label="Status:" enable-old-support>
                @foreach ($orderStatuses as $orderStatus)
                    <option value="{{ $orderStatus->id }}"
                        {{ isset($order->order_status_id) && $order->order_status_id == $orderStatus->id ? 'selected' : '' }}>
...
<a href="{{ route('admin.customers.orders.index', $order->contact_id, $order->id) }}" class="btn btn-warning"><i
        class="fa fa-times"></i> Voltar</a>

<form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" class="frm-delete"
