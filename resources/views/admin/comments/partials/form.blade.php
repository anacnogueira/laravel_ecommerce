...
<x-adminlte-select name="product_id" label="Produto:*" enable-old-support>
    @foreach ($products as $product)
        <option value="{{ $product->id }}"
            {{ isset($comment->product_id) && $product->id == $comment->product_id ? 'selected' : '' }}>
            {{ $product->name }}</option>
    @endforeach
</x-adminlte-select>
...
<x-adminlte-input-slider name="rate" label="Nota:*" min=0 max=5 step=1 color="#3c8dbc"
    value="{{ $comment->rate ?? 0 }}" enable-old-support />
...
<x-adminlte-input-switch name="status" label="status" data-on-color="success" data-off-color="danger"
    data-on-text="Ativo" data-off-text="Inativo" :config="$config" checked="$config['state']" enable-old-support />
