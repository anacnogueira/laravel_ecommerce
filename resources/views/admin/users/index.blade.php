...
@section('content')
    ...
    @php
        ...
        foreach($users  as $key => $user) {
            $status = $statusChange->returnStatusBullets("users", $user->status, $user->id);
            $data[$key] = [
                $user->id,
                $user->name,
                $user->email,
                $user->group->name,
                $status,
                '<nobr><a href="'. route('admin.users.edit', $user->id).'" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar">
                ...
                <a href="'.route('admin.users.show', $user->id) .'" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Detalhes">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary card-outline">
                <div class="card-header"> <a href="{{ route('admin.users.create') }}" class="btn btn-sm btn-primary">
                    <i class="fa fa-lg fa-fw fa-file"></i> Adicionar
                </a></div>
                <div class="card-body">
                    <x-adminlte-datatable id="table-user" :heads="$heads" head-theme="light" hoverable bordered with-buttons>
