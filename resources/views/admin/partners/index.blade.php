...
@section('title', 'Listar Parceiros')
...
@section('content')
    ...
    @php
        ...
        foreach($partners  as $key => $partner) {
            $status = $statusChange->returnStatusBullets("contacts", $partner->status, $partner->id);
            $data[$key] = [
                $partner->id,
                $partner->name,
                $partner->fantasy_name,
                $partner->cnpj,
                "<div>{$partner->email}<br>
                $partner->phone<br>
                $partner->mobile</div>",
                $status,
                '<nobr><a href="'. route('admin.partners.edit', $partner->id).'" class="btn btn-xs btn-default text-primary mx-1 shadow" title="Editar">
                ...
                <a href="'.route('admin.partners.show', $partner->id) .'" class="btn btn-xs btn-default text-teal mx-1 shadow" title="Detalhes">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card card-primary card-outline">
                <div class="card-header"> <a href="{{ route('admin.partners.create') }}" class="btn btn-sm btn-primary">
                    <i class="fa fa-lg fa-fw fa-file"></i> Adicionar
                </a></div>
                <div class="card-body">
                    <x-adminlte-datatable id="table-partner" :heads="$heads" head-theme="light" hoverable bordered with-buttons>
